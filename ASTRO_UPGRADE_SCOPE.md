...
- **Not a fire — but "build-time only" is no longer the reason. Corrected 2026-08-26.** As scoped on 2026-08-09 the remaining vulns were all **build-time tooling** (sharp/libvips image optimization the site never invokes, the esbuild bundler). A **runtime** advisory has since landed against the pinned Astro 5.18.2: **XSS in `define:vars` via incomplete `</script>` sanitization** (high). The hub uses `define:vars` in **six** places — `Feedback.astro`, `Doorstep.astro`, `records.astro`, `rentals/rights.astro`, `appointments/seats.astro`, and `almanac/index.astro` (added 2026-08-26 by `p2-t2`) — so "all remaining vulns are build-time tooling" is simply not true any more.
  **It is still not an incident, and the reason is structural, not luck:** the hub is a **fully static build** — no adapter, no `output: server`, no `prerender = false` anywhere — so every page is prerendered and **there is no request-derived data in the system at all**. Everything reaching `define:vars` is repo data baked in at build (`site.feedbackEndpoint`, the geocoder URL, `jurisdiction-facts.json`, the records datasets, the seat capture dates). `Feedback.astro` takes reader input in the browser and POSTs it; it never templates it. The exploit needs attacker-controlled data reaching that API, and on a static site there is no path for it.
  **The sixth usage needs its own sentence, because it is the first page that takes reader input at all.** `/almanac` (added 2026-08-26) has an address box. Its `define:vars` payload is `{ LOOKUP, BBOX, AUTHORITY }` — all three read from `src/data/almanac/almanac.json`, which is generated at build from the content records in `100 IPS Electify/`, so it is repo data like every other payload above. **The reader's address never reaches `define:vars`, or any server-rendered template.** It is read from the DOM input at runtime, passed to `fetch` against the jurisdiction's own GIS, and discarded — never written to storage, the URL, or the page's HTML. So the structural argument survives this page intact, but it now has to be *argued* rather than assumed: "no request-derived data in the system at all" is no longer true of the codebase, only of what gets templated. A future page that bakes a reader-supplied or third-party value into a payload breaks it for real.

  **Re-evaluate immediately if either changes:** the site adopts SSR or an adapter (or any page sets `prerender = false`), **or** a third-party/reader-supplied value starts being baked into a `define:vars` payload at build. Either turns this from planned maintenance into a fix-now.
