...
| 1.6 | **DC Appointments Watch → live** — the seat clock is built, WCAG AA verified and redteamed. **Path is `/appointments/seats`, not `/appointments`** (decision D1); the root 404s on purpose | 🟡 built, committed, **excluded from deploy** | Pippa's go + her verdicts on the redteam punch-list (D1–D5 in `REDTEAM_2026-08-17_appointments-preship.md`); a screen-reader pass has not run, and F2/F5 came back DELIBERATE | **FIVE** changes made **together**, not four: card set to `live` with `url: /appointments/seats`, sitemap entry added, the `Disallow` line removed from `robots.txt`, **the `appointments/*` and `og/appointments.png` excludes removed from `deploy-aws.sh`**, then deploy. The deploy exclude was added 2026-08-19 and is the only one of these that is a real control — the other three are signposting a determined reader walks past. Tracked as p6-t8 in the node's own roadmap. **New dependency, 2026-08-25:** the Phase 4 feedback control is in the **shared footer**, so it is on this page too (verified in the build; the page itself is still held from deploy and returns 403 live). That gives a page which names living people a public free-text intake that can receive third-party claims *about* those people — see **F9** in `REDTEAM_2026-08-25_feedback-preship.md`. **Two things now gate this card that did not before:** the node's person-naming redteam (p6-t1) has to cover the intake channel and not only the page, and the button is a third correction route that § 05 does not acknowledge, which collides with decision D3 ("GitHub now, email when ready"). Raised by the DC Appointments Watch session, not by this one | **Sharpened same day: the self-contradiction is already built, not prospective** — the page as it stands carries two correction routes while § 05 says the route is GitHub, so it contradicts itself the moment it goes live. F9 lays out the three options (page opts out and keeps GitHub · button stays and § 05 names both routes · button becomes the primary route). **Pippa's decision, deliberately not built either way** | |
...
| 4.9 | **Ship gate** — the endpoint accepts public text | ✅ **SHIPPED 2026-08-25** | 4.5, 4.6 | Redteam pass before it is live: spam flood (throttle + honeypot + caps), storage-cost blowup, someone pasting PII into the box (cannot be prevented — warn at the point of typing and never publish), and promise-consistency with the site-wide "nothing you type leaves your browser". Private-only storage defuses abusive content, since none of it ever renders. Then AA re-verify, then deploy. **Redteam done 2026-08-25 — `REDTEAM_2026-08-25_feedback-preship.md`.** Eight findings; F8 (a write-back race that would have silently erased submissions) was fixed during the build. **F1 and F3 were fixed 2026-08-25, before the endpoint exists** — F1 by evicting only already-decided items, suppressing byte-identical repeats (which is what actually blunts a flood, since eviction alone cannot help when every item is undecided), and recording `full_since` so a full inbox is the loudest thing `feedback-check` says instead of being invisible; F3 by deleting spam outright, sweeping anything decided more than 180 days ago, and saying the retention period in the panel on the site. F2/F4/F5/F6/F7 are recorded as accepted or already handled and none blocks the gate. The file carries a ship checklist, including confirming a GET returns 405 and confirming the doorstep on the production origin. **Deployed and verified live.** Endpoint `ouaejyqbxi` · bucket `inplainsight-feedback-3447` (all four public-access blocks true, AES256) · CORS locked to the two production origins and confirmed rejecting a third · **GET returns 405** · repeat suppression, honeypot and the length floor all confirmed against the live endpoint · `npm run feedback` → review page → `--apply` with a spam verdict deleted outright, through the conditional write-back, against real S3. **The doorstep's happy path ran on the production origin for the first time and works:** `3114 Sherman Ave NW` → Ward 1 · ANC 1A · SMD 1A10, one click through to `/trash` which arrived pre-filled and already looked up (Tuesday/Friday, recycling Tuesday). A real feedback submission from the live page landed in the private inbox with the right page path; both it and every verification message were then deleted, so the inbox holds nothing but what real people send. `/appointments/seats/` still returns **403** — that gate held through the deploy | |
...
- **The geocode proxy's ceiling — measured 2026-08-25, and the earlier framing here was more alarming than the numbers deserve.** It throttles at 5 req/s sustained / burst 10, behind an account Lambda concurrency cap of **10**. Measured call latency is 0.10–0.57s, so at the full 5 req/s average concurrency is ≈1.8 — about **6× headroom** before the account cap binds, and the gateway throttle is the *better* place to shed load anyway (a clean 429 beats a Lambda concurrency error). **So this is not a blocker for shipping the doorstep, and it was not.** The thing to watch before a real outreach push is the **account concurrency cap of 10**, which needs an AWS support request, not the gateway setting — a change at the gateway alone would just move the failure downstream.
