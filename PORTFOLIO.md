...
- **DC Rentals** — the shipped product is `plain-sight-rentals/` *inside* the `new rental` folder (the Cowork/Claude plugin). The **no-plugin static site** was built directly in this hub reading the law brain in place — **three surfaces are live: `/rentals/rights`, `/rentals/scam-check`, and `/rentals/lease-check`** (the read-your-lease checker, shipped 2026-08-08 with a live-statute redteam pass). All three generate their legal content at build time from the shared law brain via `scripts/sync-law-data.mjs` (the JSON-twin drift-proof pattern), so a page can't silently drift from the law. UI copy is American English (checker verbs Americanized 2026-08-08, ahead of future translation). Next candidate surface: `move-in` (deposit-protecting checklist + printable condition inventory). (A 2026-07-02 plan to first export a clean `plain-sight-law/` folder was superseded and its records corrected 2026-08-08; that folder was never persisted and is not needed — the `law/` topic files in `plain-sight-rentals/` are the single source of truth.)
